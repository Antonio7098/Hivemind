...
run "$HIVEMIND" project runtime-set sprint59-graph-runtime --role worker --adapter native --binary-path builtin-native --model "$MODEL_ID" --timeout-ms 180000 --env HIVEMIND_NATIVE_PROVIDER=openrouter --env HIVEMIND_NATIVE_CAPTURE_FULL_PAYLOADS=true --env HIVEMIND_GRAPH_QUERY_PYTHON="$GRAPH_QUERY_PYTHON_BIN" --env HIVEMIND_NATIVE_MAX_TURNS=20
...
2) ACT:tool:graph_query:{"kind":"python_query","repo_name":"repo","max_results":8,"limits":{"max_seconds":2.0,"max_operations":24,"max_trace_events":1200,"max_stdout_chars":600},"code":"files = graph.find(node_class='file', path_regex=r'src/native/tool_engine/graph_query_tool/(execute|types)\\.rs', limit=4)\nfor node in files[:2]:\n    session.add(node, detail='summary')\nresult = {'files': [node['logical_key'] for node in files]}"}
3) ACT:tool:read_file:{"path":"src/native/tool_engine/graph_query_tool/execute.rs"}
4) ACT:tool:graph_query:{"kind":"python_query","repo_name":"repo","max_results":8,"limits":{"max_seconds":2.0,"max_operations":24,"max_trace_events":1200,"max_stdout_chars":600},"code":"symbols = graph.find(node_class='symbol', name_regex='GraphQueryInput|GraphQueryResult|RuntimeGraphSnapshotRepository', limit=6)\nfor symbol in symbols[:2]:\n    session.add(symbol, detail='summary')\nresult = {'symbols': [node['logical_key'] for node in symbols]}"}
...
ATTEMPT_COUNT="$(printf '%s' "$FLOW_STATUS_JSON" | jq -r --arg task_id "$TASK_ID" '.data.task_executions[$task_id].attempt_count // 0')"
TURN_BUDGET_FAILURES="$(jq -r --arg flow_id "$FLOW_ID" "$FLOW_EVENTS_FILTER | map(.payload) | map(select((.type==\"runtime_error_classified\" and .code==\"native_turn_budget_exceeded\") or (.type==\"agent_invocation_completed\" and .error_code==\"native_turn_budget_exceeded\"))) | length" "$EVENTS_PATH")"
assert_eq "$FLOW_STATE" "completed" "flow terminal state"
case "$TASK_STATE" in
  success|completed) ;;
  *) echo "ASSERT FAILED [task execution state]: expected success/completed got '$TASK_STATE'" >&2; exit 1 ;;
esac
assert_eq "$TURN_BUDGET_FAILURES" "0" "turn budget failures"
...
- Turn-budget failures observed: ${TURN_BUDGET_FAILURES}
