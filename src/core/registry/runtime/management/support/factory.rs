...
const RUNTIME_ADAPTER_BUILDERS: &[(&str, RuntimeAdapterBuilder)] = &[
    ...
    ("claude-code", build_claude_code_adapter),
    ("kilo", build_kilo_adapter),
    ("native", build_native_adapter),
];
...
fn parse_native_agent_mode(
    ...
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "task_executor" | "task-executor" | "executor" => crate::native::AgentMode::TaskExecutor,
        _ => fallback,
    }
}
...
fn parse_native_runtime_args(cfg: &mut NativeAdapterConfig, args: &[String]) -> Vec<String> {
    ...
    while index < args.len() {
        ...
        if let Some(raw) = arg.strip_prefix("--max-turns=") {
            ...
            cfg.native.agent_mode = parse_native_agent_mode(raw, cfg.native.agent_mode);
...
fn build_native_adapter(runtime: ProjectRuntimeConfig) -> SelectedRuntimeAdapter {
    ...
    let scripted_from_args = parse_native_runtime_args(&mut cfg, &runtime_args);
    ...
    {
        serde_json::from_str::<Vec<String>>(raw).unwrap_or_else(|_| cfg.scripted_directives.clone())
...
impl Registry {
    pub(crate) fn build_runtime_adapter(
        ...
        let Some((_, build)) = RUNTIME_ADAPTER_BUILDERS
            .iter()
            .find(|(adapter_name, _)| *adapter_name == runtime.adapter_name.as_str())
        else {
