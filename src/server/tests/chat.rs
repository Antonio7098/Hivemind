...
fn api_chat_invoke_ok_with_mock_provider() {
    ...
    let resp = handle_api_request_inner(&app, ApiMethod::Post, "/api/chat/invoke", 10, Some(&body))
...
fn api_chat_sessions_create_send_and_inspect_round_trip() {
    ...
    let inspect_resp = handle_api_request_inner(&app, ApiMethod::Get, &inspect_path, 10, None)
