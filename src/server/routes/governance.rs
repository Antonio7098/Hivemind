...
pub(super) fn handle_get(path: &str, url: &str, app: &AppContext) -> Result<Option<ApiResponse>> {
    let governance_service = || app.governance_service();
    let resp = match path {
        "/api/governance/constitution" => {
            let query = parse_query(url);
            let project = query.get("project").map_or("", |s| s.as_str());
            super::json_ok(governance_service()?.constitution_show(project)?)?
        }
        "/api/governance/documents" => {
            let query = parse_query(url);
            let project = query.get("project").map_or("", |s| s.as_str());
            super::json_ok(governance_service()?.project_governance_document_list(project)?)?
        }
        "/api/governance/documents/inspect" => {
            ...
            super::json_ok(
                governance_service()?.project_governance_document_inspect(project, document_id)?,
            )?
        }
        "/api/governance/notepad" => {
            let query = parse_query(url);
            let project = query.get("project").map_or("", |s| s.as_str());
            super::json_ok(governance_service()?.project_governance_notepad_show(project)?)?
        }
        "/api/governance/global/notepad" => {
            super::json_ok(governance_service()?.global_notepad_show()?)?
        }
        "/api/governance/global/skills" => {
            super::json_ok(governance_service()?.global_skill_list()?)?
        }
        "/api/governance/global/skills/inspect" => {
            let query = parse_query(url);
            let skill_id = query.get("skill_id").map_or("", |s| s.as_str());
            super::json_ok(governance_service()?.global_skill_inspect(skill_id)?)?
        }
        "/api/governance/global/templates" => {
            super::json_ok(governance_service()?.global_template_list()?)?
        }
        "/api/governance/global/templates/inspect" => {
            let query = parse_query(url);
            let template_id = query.get("template_id").map_or("", |s| s.as_str());
            super::json_ok(governance_service()?.global_template_inspect(template_id)?)?
        }
        _ => return Ok(None),
    };
    ...
    let resp =
        match path {
            "/api/governance/constitution/check" => {
                ...
                super::json_ok(governance_service()?.constitution_check(&req.project)?)?
            }
            "/api/governance/graph-snapshot/refresh" => {
                ...
                super::json_ok(graph_service()?.graph_snapshot_refresh(
                    &req.project,
